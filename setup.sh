...
10.98.0.4 <PeerVPNIP> : PSK "ThisIsNotASecurePSK123"
EOF
...
sudo ip route add 10.97.0.0/16 via 10.98.1.1 dev eth1
sudo ip route add 168.63.129.16 via 10.98.1.1 dev eth1 proto dhcp src 10.98.1.4
sudo ip tunnel add vti0 local 10.98.0.4 remote <PeerVPNIP> mode vti key 12
sudo sysctl -w net.ipv4.conf.vti0.disable_policy=1
sudo ip link set up dev vti0
sudo ip route add <Your_on_prem_network> dev vti0 # Route to on-prem
' | sudo tee -a /etc/rc.local && sudo chmod +x /etc/rc.local
